...
The log level can be set using the `service-log-level` input. The default is `debug`, which will
...
| <code>service-log-level</code> | The log level used for Docker Compose service logs. Can be one of "debug", "info".                                                               | debug                                | **false**    |
